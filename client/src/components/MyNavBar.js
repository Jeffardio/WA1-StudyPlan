...
function MyNavBar(props){
    ...
    async function redirect(){
        if(fun === 'Login'){
            ...
            if (await props.handleLogout())
                ...
                props.setShowErr(new Error("Error: something went wrong during the logout procedure, try again... :("))
        }
        else {
            navigate('/')
        }
    ...
    const name = props.user ? <div><b>Welcome back, {props.user.name}!</b></div> : ''
return (
    <nav className="navbar navbar-dark bg-success">
        <Container fluid>
            ...
            <Button variant="danger" onClick = {() => {redirect()}}>{fun}</Button>
